...
def read_csv(file_path):
    with open(file_path, 'r', encoding='windows-1252') as csv_file:  # Adjust encoding accordingly
...
def create_database(csv_data, db_file):
    ...
    create_table_query = f"CREATE TABLE IF NOT EXISTS blog_blogactivities ({', '.join([f'{column} TEXT' for column in columns])})"
    ...
    for row in csv_data[1:]:
        ...
        c.execute(f"INSERT INTO blog_blogactivities VALUES ({', '.join(['?' for _ in range(len(row))])})", row)
...
def main():
    # Path to CSV file
    csv_file_path = 'smart-trips\blog\data-article.csv'
