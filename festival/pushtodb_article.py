...
def create_database(csv_data, db_file):
    ...
    create_table_query = f"CREATE TABLE IF NOT EXISTS festival_festivalactivities ({', '.join([f'{column} TEXT' for column in columns])})"
    ...
    for row in csv_data[1:]:
        ...
        c.execute(f"INSERT INTO festival_festivalactivities VALUES ({', '.join(['?' for _ in range(len(row))])})", row)
...
def main():
    # Path to CSV file
    csv_file_path = 'smart-trips\\festival\\data-article.csv'
